...
*/
    ...
    q->b_index = 0; //init free index
    q->f_index = 0; //init first index

    for (int i = 0; i < QUEUE_SIZE; i++){   //go through array and set every element to *
        q->arr[i] = '*';
    }
    //q->arr[QUEUE_SIZE-1] = '\0';    //end array //TODO - check if important or working
}
...
*/
	...
	return (index+1) % QUEUE_SIZE;  //next element and cycle via modulo
}
...
*/
	...
	return q->b_index == q->f_index ? 1 : 0; //if first free is the first element, queue is empty
}
...
*/
    ...
    return nextIndex(q->b_index) == q->f_index ? 1 : 0;    //if next element after first free is also free, queue is not full
}
...
*/
    ...
    //move first index to second element
...
*/
    ...
    queueFront(q, c);   //get first element
    queueRemove(q);     //remove first element
...
*/
    ...
    q->arr[q->b_index] = c; //write c to first free slot
    q->b_index = nextIndex(q->b_index); //move free to next free slot
}
