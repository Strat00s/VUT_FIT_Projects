...
/*                                                                            */
/* Reseni vytvoril(a) a odevzdava: (Lukas Bastyr)                             */
/*                                                                            */
/******************************************************************************/
...
void ADC0_Init(void) {
    ...
    ADC0_CFG1 = (ADC_CFG1_ADIV(3) | ADC_CFG1_MODE(0) | ADC_CFG1_ADICLK(1));
...
void ADC0_IRQHandler(void) {
    if (ADC0_SC1A & ADC_SC1_COCO_MASK) {
    	float val = ((ADC0_RA * 3.3) / 255.0);
        int before_dot = val;
        int after_dot = val * 1000 - before_dot * 1000;
        sprintf(result, "%d.%03d", before_dot, after_dot);
